...
PyTorch implementation of hierarchical deep temporal models for **group activity recognition in volleyball videos**,
inspired by the CVPR 2016 paper [*A Hierarchical Deep Temporal Model for Group Activity
Recognition*](https://arxiv.org/abs/1511.06040).

This project explores how group activity recognition can be improved by moving from single-frame classification to
player-level temporal modeling and hierarchical team-level aggregation. Group activity recognition is challenging
because the final group label depends not only on individual visual features, but also on interactions between players
and temporal context.

The repository includes multiple baselines and experimental variants, ending with a left/right hierarchical LSTM model
that preserves team-side structure and achieves the best reported result in this repository.

---

## Highlights

- Implements multiple deep learning baselines for volleyball group activity recognition.
...
- Includes frame-level classification, player-action classification, temporal frame modeling, temporal player modeling,
  and hierarchical LSTM models.
- Provides reusable dataset modes through `VolleyballDataset`.
- Includes training scripts, evaluation scripts, classification reports, and confusion matrix generation.
...
- B8 is the strongest reported variant because it keeps the left/right team structure while modeling temporal player
  features.
- Results may depend on preprocessing choices, split definitions, training settings, and checkpoint availability.
...
```text
scripts/script_constants.py
```

On Kaggle, the code detects `KAGGLE_KERNEL_RUN_TYPE` and expects paths similar to:
...
Outside Kaggle, update the paths manually before running experiments.
...
Some checkpoint-dependent scripts also contain hard-coded checkpoint paths. Update them before running:

- `scripts/b3_b_script.py` expects a trained B3A checkpoint.
- `scripts/b4_script.py` may load a B4 checkpoint before training or evaluation.
...
2. Creates train, validation, and test dataloaders.
...
The B-numbering follows the internal experiment sequence used during implementation. Each model adds or modifies one
component, such as player crops, temporal modeling, feature pooling, or hierarchical aggregation.
...
A ResNet50 ImageNet backbone with a dropout and linear classification head for 8 group activity classes.
...
The best reported model in this repository. It follows the hierarchical structure of B7 but preserves team-side
structure by pooling slots `0:6` and `6:12` separately before the frame-level LSTM.
...
Group activity labels, as defined in the code:
...
because they process tensors shaped like:
...
- Model checkpoints are not included because of file size limitations.
- Some scripts require local path configuration before running.
- Some checkpoint-dependent scripts include hard-coded checkpoint paths that should be updated before reproduction.
- Results are reported from the committed evaluation reports and may depend on preprocessing, split definitions, and
  training settings.
- This repository focuses on implementing and exploring hierarchical temporal modeling ideas rather than providing a
  fully optimized production pipeline.
